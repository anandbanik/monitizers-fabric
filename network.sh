...
CHAINCODE_CUSTOMER_WALX=customer
CHAINCODE_CUSTOMER_WALX_INIT='{"Args":["init"]}'
...
DEFAULT_ORDERER_EXTRA_HOSTS="extra_hosts:[newline]      - peer0.$ORG1.$DOMAIN:$IP1[newline]      - peer0.$ORG2.$DOMAIN:$IP2[newline]"
...
function removeDockersFromCompose() {
    #for o in ${DOMAIN} ${ORG1} ${ORG2} ${ORG3} ${ORG4} "util"
    for o in ${DOMAIN} ${ORG1} ${ORG2} "util"
    do
